...
#' @param norm.method The microarray-style normalization method to be applied to the logCPM values. Choices are as for the method argument of voom function.
#' @references
...
voom.qn.limma.createRmd <- function(data.path, result.path, codefile, norm.method='quantile') {
  ...
  writeLines(c("is.valid <- check_compData(cdata)",
               ...
               paste("voom.data <- limma::voom(count.matrix(cdata), design = model.matrix(~factor(sample.annotations(cdata)$condition)), normalize.method = '", norm.method, "')", sep = ''),
