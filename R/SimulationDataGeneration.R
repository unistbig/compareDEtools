...
#' @param mean.condition read count vector of a certain condition(normal/cancer)
#' @param disp.condition dispersion vector of a certain condition(normal/cancer)
...
#' @param samples.per.cond An integer indicating number of samples for each condition (e.g. 3).
...
#' @param dispType Characters indicating how is the dispersion parameter assumed to be for each condition to make a synthetic data. Possible values are 'same' and 'different'.
...
#' @param dataset Characters specifying the file name of simulation dataset.
#' @param samples.per.cond An integer indicating number of samples for each condition (e.g. 3).
#' @param fpc A logical indicating whether simulation data is made from single sample condition(e.g. normal) to calculate False positive counts. Default is FALSE.
#' @param dataset.parameters A list containing estimated mean and dispersion parameters and filtered count from original count dataset.
#' @export
